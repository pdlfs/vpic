...
vpic_simulation::initialize( int argc,
  ...
  if( rank()==0 ) VMESSAGE(( "Checking interdomain synchronization" ));
  TIC err = FAK->synchronize_tang_e_norm_b( field_array ); TOC( synchronize_tang_e_norm_b, 1 );
  if( rank()==0 ) VMESSAGE(( "Error = %e (arb units)", err ));

  if( rank()==0 ) VMESSAGE(( "Checking magnetic field divergence" ));
  ...
  if( rank()==0 ) VMESSAGE(( "RMS error = %e (charge/volume)", err ));
  TIC FAK->clean_div_b( field_array ); TOC( clean_div_b, 1 );

  // Load fields not initialized by the user

  if( rank()==0 ) VMESSAGE(( "Initializing radiation damping fields" ));
  TIC FAK->compute_curl_b( field_array ); TOC( compute_curl_b, 1 );

  if( rank()==0 ) VMESSAGE(( "Initializing bound charge density" ));
  ...
  if( rank()==0 ) VMESSAGE(( "Checking electric field divergence" ));
  ...
  if( rank()==0 ) VMESSAGE(( "Rechecking interdomain synchronization" ));
  TIC err = FAK->synchronize_tang_e_norm_b( field_array ); TOC( synchronize_tang_e_norm_b, 1 );
  if( rank()==0 ) VMESSAGE(( "Error = %e (arb units)", err ));
    
  if( species_list ) {
    if( rank()==0 ) VMESSAGE(( "Uncentering particles" ));
  ...
  if( rank()==0 ) VMESSAGE(( "Performing initial diagnostics" ));
  ...
  if( rank()==0 ) VMESSAGE(( "Initialization complete" ));
